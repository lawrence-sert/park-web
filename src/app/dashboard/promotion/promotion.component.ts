...
export class PromotionComponent implements OnInit {
	...
	promotionRef!: AngularFirestoreCollection<Promotions>;
	promotion$!: Observable<Promotions[]>;
	promotionID: any[] = [];
  ...
  constructor(
		...
		this.activatedRoute.params.subscribe(parameter => {
			this.parameterValue = parameter.promotionID
		});
	}

  ngOnInit(): void {
  	this.promotionRef = this.db.collection<{}>('promotions', ref => ref.where('id', '==', this.parameterValue));
		this.promotion$ = this.promotionRef.snapshotChanges().pipe(
  ...
  async uploadImagetoFirebase(event:any){
		...
		const path =  `promotions/${new Date().getTime()}_${fileName.name}`;
		...
		this.imageUpload.then( res=>{
			...
			imagefile.then( downloadableUrl=>{
				...
				this.db.doc(`promotions/${param}`).update({
					promotion_image: downloadableUrl
				});
			})
		})
	}
